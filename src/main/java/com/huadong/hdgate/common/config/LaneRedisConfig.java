...
public class LaneRedisConfig {
    ...
    RedisMessageListenerContainer lanecontainer(RedisConnectionFactory connectionFactory,
        ...
        //订阅了一个叫crane 的通道
        container.addMessageListener(lane1listenerAdapter, new PatternTopic("test")); // hd_gate_business_data_db 频道用来业务数据的数据库操作，避免死锁
     ...
     */
    @Bean
    MessageListenerAdapter lane1listenerAdapter(MessageReceiver receiver) {
    ...
    /**redis 读取内容的template */
    @Bean(name="Lane1Template")
    StringRedisTemplate template(RedisConnectionFactory connectionFactory) {
        return new StringRedisTemplate(connectionFactory);
    }

    @Bean(name="redisPoolFactory1")
    public JedisPool redisPoolFactory()  throws Exception{
        ...
        JedisPool jedisPool = new JedisPool(jedisPoolConfig, host, port, 10000, password, 1);
        return jedisPool;
    }
}
